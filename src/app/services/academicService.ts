...
import { Atividade, Boletim, Correcao, Disciplina, Entrega, EventoCronograma, ListaPaginada, RespostaQuestao, Turma } from '../types/api';
...
export const createEntrega = (atividadeId: string, data: { resposta?: string; respostas?: RespostaQuestao[] }) =>
  apiFetch<{ mensagem: string; entrega: Entrega }>(`/atividades/${atividadeId}/entregas`, { method: 'POST', body: JSON.stringify(data) });
...
export const createEventoCronograma = (data: Omit<EventoCronograma, '_id' | 'professorId'>) =>
  apiFetch<{ mensagem: string; evento: EventoCronograma }>('/cronograma', { method: 'POST', body: JSON.stringify(data) });
export const updateEventoCronograma = (id: string, data: Partial<Omit<EventoCronograma, '_id' | 'professorId'>>) =>
  apiFetch<{ mensagem: string; evento: EventoCronograma }>(`/cronograma/${id}`, { method: 'PUT', body: JSON.stringify(data) });
export const deleteEventoCronograma = (id: string) =>
  apiFetch<{ mensagem: string }>(`/cronograma/${id}`, { method: 'DELETE' });
